...
(function($) {
    ...
    // 验证是否引用jquery
    if (!$ || !$.fn || !$.fn.jquery) {
        alert('在引用wangEditor.js 之前，先引用jQuery，否则无法使用 wangEditor');
        return;
    }
    ...
    _e(function(E, $) {
        ...
        E.createMenu(function(check) {
            ...
            var menu = new E.Menu({
                ...
                id: menuId, // 菜单id
                title: '日期', // 菜单标题
                commandName: 'calendar', // 命令名称

                // 正常状态和选中装下的dom对象，样式需要自定义
                $domNormal: $('<a href="#" tabindex="-1"><i class="wangeditor-menu-img-calendar"></i></a>'),
                $domSelected: $('<a href="#" tabindex="-1" class="selected"><i class="wangeditor-menu-img-calendar"></i></a>')
            });
            ...
            menu.clickEvent = function(e) {
                ...
                var date = new Date();
                var calendar = date.getFullYear() + "-" + returnTwo((parseInt(date.getMonth()) + 1).toString()) + "-" + returnTwo(date.getDate());

                // 使用自定义命令
                function commandFn() {
                    editor.command(e, 'insertHtml', calendar);
                }
                editor.customCommand(e, commandFn);
            };
    ...
    _e(function(E, $) {
        ...
        E.createMenu(function(check) {
            ...
            var menu = new E.Menu({
                ...
                id: menuId, // 菜单id
                title: '日期', // 菜单标题
                commandName: 'time', // 命令名称

                // 正常状态和选中装下的dom对象，样式需要自定义
                $domNormal: $('<a href="#" tabindex="-1"><i class="wangeditor-menu-img-time"></i></a>'),
                $domSelected: $('<a href="#" tabindex="-1" class="selected"><i class="wangeditor-menu-img-time"></i></a>')
            });
 ...
 * 将一位的数字字符串自动补全成两位
 * @private
 * @param  {string} index 临时字符串变量
 * @return {string}       补全好的字符串变量
 */
